...
*/
	function addData(){
		...
		for(var idx=0; idx<3; idx++){
			addTab += "<tr><td>"+plist1[idx]+"</td><td>"+plist2[idx]+"</td></tr>";
		}
...
*/
function Student(stdnum, kor, eng, math){
	this.stdnum = stdnum;
	this.kor = kor;
	this.eng = eng;
	this.math = math;
	...
	this.toString = function(){
		return 'stdnum: '+ this.stdnum +'\t평균: '+ this.avg();
	}
}
var stdArray = [];

for(idx=1; idx<=30; idx++){
	stdArray.push(new Student(idx,Math.floor(Math.random()*100+1),Math.floor(Math.random()*100+1),Math.floor(Math.random()*100+1)))
}
console.log(stdArray);
stdArray = stdArray.sort(function(left, right){
	return right.avg() - left.avg();
}).slice(0,3);
var output ='';
for(var i in stdArray){
	output+= stdArray[i].toString()+'\n';
}
console.log(output);
/*
[하] 5. 배열 삭제 메서드의 prototype선언과 삭제 위치에 대한 역순위 처리의 이유에 대하여 기술하세요.
 		> splice는 이후의 내용으로 삭제가 되기 때문
 */
</script>
</html>